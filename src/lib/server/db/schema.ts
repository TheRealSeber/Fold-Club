...
export const productImages = pgTable(
  ...
  (table) => [uniqueIndex('pi_product_display_order_idx').on(table.productId, table.displayOrder)]
);
...
export const orders = pgTable(
  'orders',
  {
    ...
    trackingSessionId: uuid('tracking_session_id').references(() => trackingSessions.id, {
      onDelete: 'set null'
    }),
...
export const consentRecords = pgTable(
  ...
  (table) => [index('cr_session_id_idx').on(table.sessionId)]
);
